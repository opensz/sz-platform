 ...
 * 
 * @author bobo modified for SzPlatform
 */
class HtmlCoreReport extends HtmlAbstractReport {
	...
	private void writeSummary() throws IOException {
		...
		final String javaMelodyUrl = "JavaMelody";
	...
	void writeMessageIfNotNull(String message, String partToRedirectTo, String anchorNameForRedirect)
		...
		if (message != null) {
			writeln(SCRIPT_BEGIN);
			// writeDirectly pour ne pas g茅rer de traductions si le message contient '#'
			writeDirectly("alert(\"" + javascriptEncode(message) + "\");");
			writeln("");
			// redirect vers une url 茅vitant que F5 du navigateur ne refasse l'action au lieu de faire un refresh
	...
	private void writeGraphs(Collection<JRobin> jrobins) throws IOException {
		...
		for (final JRobin jrobin : jrobins) {
			...
			i++;
			if (i % 3 == 0) {
				// un <br/> apr猫s httpSessions et avant activeThreads pour l'alignement
				writeln("<br/>");
			}
		}
	}
	...
	private void writeCurrentRequests(List<ThreadInformations> threadInformationsList,
			...
			Map<String, HtmlCounterReport> counterReportsByCounterName) throws IOException {
		// counterReportsByCounterName peut 锚tre null
	...
	private void writeThreads() throws IOException {
		...
		for (final JavaInformations javaInformations : javaInformationsList) {
			...
			if (threadInformationsList.size() <= MAX_THREADS_DISPLAYED_IN_MAIN_REPORT) {
				...
				// le nombre de threads d茅passe le maximum pour 锚tre affich茅 dans le rapport
				// principal, donc on affiche un lien vers une page � part
	...
	private void writeSystemActionsLinks() throws IOException {
		...
		if (serverInfo != null && !serverInfo.contains("Winstone")) {
			// on n'affiche pas le lien JNDI si serveur Winstone car cela n'a pas d'int茅r锚t
	...
	void writeRefreshAndPeriodLinks(String graphName, String part) throws IOException {
		...
		// On affiche des liens vers les p茅riodes.
		// Rq : il n'y a pas de p茅riode ni de graph sur la derni猫re heure puisque
		// si la r茅solution des donn茅es est de 5 min, on ne verra alors presque rien
