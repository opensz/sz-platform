...
public class BpmFormRule extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.id, rhs.id)
				...
				.append(this.memo, rhs.memo).append(this.tipInfo, rhs.tipInfo)
				.isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.id)
				.append(this.name).append(this.rule).append(this.memo)
				.append(this.tipInfo).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("id", this.id)
				.append("name", this.name).append("rule", this.rule)
				.append("memo", this.memo).append("tipInfo", this.tipInfo)
				.toString();
	}
}
