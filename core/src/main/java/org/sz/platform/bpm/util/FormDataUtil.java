...
public class FormDataUtil {
	...
	public static List<SqlModel> parseSql(BpmFormData formData) {
		...
		SqlModel sqlModel;
		if (isAdd) {
			sqlModel = getInsert(formData.getTableName(),
					formData.getMainFields());
			list.add(sqlModel);
		} else {
	...
	public static BpmFormData parseJson(String json, PkValue pkValue)
			throws Exception {
		BpmFormTableDao bpmFormTableDao = (BpmFormTableDao) ContextUtil
				.getBean("bpmFormTableDao");
		BpmFormFieldDao bmpFormFieldDao = (BpmFormFieldDao) ContextUtil
				.getBean("bpmFormFieldDao");
		...
		BpmFormTable mainTableDef = (BpmFormTable) bpmFormTableDao.getById(Long
				.valueOf(tableId));

		String tableName = mainTableDef.getTableName();

		List listTable = bpmFormTableDao.getSubTableByMainTableId(Long
				.valueOf(tableId));
	...
	private static void handleMain(JSONObject jsonObj, BpmFormData bpmFormData,
			BpmFormTable mainTableDef, BpmFormFieldDao bmpFormFieldDao)
			throws Exception {
		JSONObject mainTable = jsonObj.getJSONObject("main");
		...
		String tablePrefix = isExternal == 1 ? ""
				: TableModel.CUSTOMER_TABLE_PREFIX;

		String colPrefix = isExternal == 1 ? ""
		...
		Map mainFiledsData = handleRow(colPrefix, mainFieldTypeMap,
				mainFieldJson);
		...
		if (pkValue.getIsAdd()) {
			...
			Map map = caculateField(colPrefix, mapFormField,
	...
	private static void handSub(JSONObject jsonObj,
			List<BpmFormTable> listTable, BpmFormFieldDao bmpFormFieldDao,
			BpmFormTable mainTableDef, BpmFormData bpmFormData)
			throws Exception {
		...
		for (int i = 0; i < arySub.size(); i++) {
			...
			if (subTableId == null)
				continue;
			BpmFormTable bpmFormTable = (BpmFormTable) formTableMap
					.get(subName);
			...
			for (int j = 0; j < arySubFields.size(); j++) {
				...
				handFkRow(mainTableDef, bpmFormTable, subRow,
						bpmFormData.getPkValue());
	...
	private static void handFkRow(BpmFormTable mainTabDef,
			BpmFormTable bpmFormTable, Map<String, Object> rowData,
			PkValue pkValue) throws Exception {
	...
	private static Map<String, Object> caculateField(String colPrefix,
			List<BpmFormField> fields, Map<String, Object> data) {
	...
	public static PkValue getPk(BpmFormTable bpmFormTable) throws Exception {
		Object pkValue = null;
		String pkField = "ID";
	...
	public static String getTableName(BpmFormTable formTable) {
	...
	public static String getOriginColumnName(String colName) {
		String originColName = null;

		if (colName != null) {
			originColName = (colName).toLowerCase().replaceFirst(
					TableModel.CUSTOMER_COLUMN_PREFIX.toLowerCase(), "");
		}
		return originColName;
	}

	private static Map<String, Object> handleRow(String colPrefix,
			Map<String, String> fieldTypeMap, JSONObject fieldsJsonObj) {
		Map row = new HashMap();
	...
	private static SqlModel getInsert(String tableName,
	...
	private static SqlModel getDel(SubTable subTable, PkValue pkValue) {
		String sql = "delete from " + subTable.getTableName() + " where "
				+ subTable.getFkName() + "=?";
