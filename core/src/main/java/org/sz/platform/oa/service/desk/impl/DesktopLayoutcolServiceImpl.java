...
public class DesktopLayoutcolServiceImpl extends
		BaseServiceImpl<DesktopLayoutcol> implements DesktopLayoutcolService {
	...
	public void setListData(List<DesktopLayoutcol> list) {
		for (DesktopLayoutcol dlc : list) {
			if (dlc.getColumnId().longValue() != 0L) {
				dlc.setColumnName(this.desktopColumnDao.getNameById(dlc
						.getColumnId().longValue()));
			}
			if (dlc.getLayoutId().longValue() != 0L)
				dlc.setLayoutName(this.desktopLayoutDao.getNameById(dlc
						.getLayoutId().longValue()));
		}
	}
	...
	public DesktopLayoutcol showData(Long id,
		...
		DesktopLayoutcol desktopLayoutcol = (DesktopLayoutcol) this.dao
				.getById(id);
		return desktopLayoutcol;
	}
	...
	public List<DesktopLayoutcol> layoutcolData(Long layoutId) {
		...
		for (DesktopLayoutcol dlc : list) {
			if (dlc.getColumnId().longValue() != 0L) {
				DesktopColumn desktopColumn = (DesktopColumn) this.desktopColumnDao
						.getById(dlc.getColumnId());
