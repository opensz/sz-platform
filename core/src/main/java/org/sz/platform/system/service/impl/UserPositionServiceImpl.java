...
public class UserPositionServiceImpl extends BaseServiceImpl<UserPosition>
	...
	public void add(Long posId, Long[] userIds) throws Exception {
		if ((posId == null) || (posId.longValue() == 0L) || (userIds == null)
				|| (userIds.length == 0))
			return;
	...
	public void delUserPosByIds(String[] lAryId, Long userId) {
		...
		for (String posId : lAryId)
			if (!StringUtil.isEmpty(posId))
				this.userPositionDao.delUserPosByIds(userId,
						Long.valueOf(Long.parseLong(posId)));
	}
