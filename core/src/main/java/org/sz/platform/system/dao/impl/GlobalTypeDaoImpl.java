...
public class GlobalTypeDaoImpl extends BaseDaoImpl<GlobalType> implements
	...
	public boolean isNodeKeyExistsForUpdate(Long typeId, String catKey,
		...
		params.put("catkey", catKey);
		params.put("nodeKey", nodeKey);
		int rtn = ((Integer) getOne("isNodeKeyExistsForUpdate", params))
				.intValue();
		return rtn > 0;
	}
	...
	public GlobalType getByDictNodeKey(String nodeKey) {
		GlobalType globalType = (GlobalType) getUnique("getByDictNodeKey",
				nodeKey);
		return globalType;
	}
	...
	public List<GlobalType> getByFormRights(String catKey, Long userId,
			String roleIds, String orgIds) {
		Map params = new HashMap();
		params.put("ownerId", userId);
		params.put("catKey", catKey);
