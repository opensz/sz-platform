...
import org.sz.platform.system.service.SysOrgService;
 ...
 public class MessageReceiverServiceImpl extends BaseServiceImpl<MessageReceiver> implements MessageReceiverService
 {
   ...
   public List<Map> getMessageReadReply(Long messageId)
   {
     ...
     List<MessageReceiver> reList = this.redao.getMessageReceiverList(messageId);
     ...
     for (MessageReceiver reModel : reList)
     {
       ...
       {
         SysOrg sysOrg = (SysOrg)this.orgSevice.getById(reModel.getReceiverId());
         if (sysOrg != null) {
           path = sysOrg.getPath();
           if (!StringUtil.isEmpty(path)) {
             listByOrg = this.dao.getReadReplyByPath(messageId, path);
