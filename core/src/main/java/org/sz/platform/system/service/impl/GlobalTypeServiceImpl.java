...
public class GlobalTypeServiceImpl extends BaseServiceImpl<GlobalType>
	...
	public GlobalType getInitGlobalType(int isRoot, long parentId)
		...
		if (isRoot == 1) {
			SysTypeKey sysTypeKey = (SysTypeKey) this.sysTypeKeyDao
					.getById(Long.valueOf(parentId));
			...
			globalType = (GlobalType) this.globalTypeDao.getById(Long
					.valueOf(parentId));
	...
	public void delByTypeId(Long typeId) {
		...
		List<GlobalType> childrenList = this.globalTypeDao
	...
	public boolean isNodeKeyExistsForUpdate(Long typeId, String catKey,
			String nodeKey) {
		return this.globalTypeDao.isNodeKeyExistsForUpdate(typeId, catKey,
				nodeKey);
	}
		...
		if (("prev".equals(moveType)) || ("next".equals(moveType))) {
			...
			for (GlobalType globalType : list) {
				if (globalType.getTypeId().equals(dragId)) {
					...
					globalType.setNodePath(target.getNodePath()
							+ globalType.getTypeId() + ".");
				} else {
					...
					String tmp = targetPath + dragged.getTypeId() + "."
							+ tmpPath;
	...
	public Set<GlobalType> getByBpmRightCat(Long userId, String roleIds,
			String orgIds, boolean hasRoot) {
		Set globalTypeSet = new LinkedHashSet();

		List<GlobalType> globalTypeList = this.globalTypeDao.getByBpmRights(
				"FLOW_TYPE", userId, roleIds, orgIds);
		...
		for (GlobalType globalType : globalTypeList) {
			if (StringUtils.isNotEmpty(globalType.getNodePath())) {
				...
				int index = parentNodePath.indexOf(globalType.getTypeId()
						.toString());
				if (index != -1) {
				...
				if (nodePaths.length >= 2) {
					for (int i = 1; i < nodePaths.length; i++) {
						GlobalType parentType = (GlobalType) this.globalTypeDao
								.getById(new Long(nodePaths[i]));
	...
	public Set<GlobalType> getByFormRightCat(Long userId, String roleIds,
			String orgIds, boolean hasRoot) {
		Set globalTypeSet = new LinkedHashSet();

		List<GlobalType> globalTypeList = this.globalTypeDao.getByFormRights(
				"FORM_TYPE", userId, roleIds, orgIds);
		globalTypeSet.addAll(globalTypeList);
		...
		if (BeanUtils.isNotEmpty(list)) {
			for (GlobalType gt : list)
				if (typeId.equals(gt.getParentId())) {
					sb.append("<folder id='" + gt.getTypeId() + "' label='"
							+ gt.getTypeName() + "'>");
	...
	public List<GlobalType> getByBpmRights(String catKey, Long userId,
			String roleIds, String orgIds) {
		return this.globalTypeDao.getByBpmRights(catKey, userId, roleIds,
				orgIds);
	}
}
