...
import org.sz.platform.system.service.UserRoleService;
 ...
 public class UserRoleServiceImpl extends BaseServiceImpl<UserRole> implements UserRoleService
 {
   ...
   public void add(Long roleId, Long[] userIds)
     throws Exception
   {
     if ((roleId == null) || (roleId.longValue() == 0L) || (userIds == null) || (userIds.length == 0)) return;
     for (Long userId : userIds)
     {
       UserRole userRole = this.userRoleDao.getUserRoleModel(userId, roleId);
       if (userRole != null)
         continue;
   ...
   public void delUserRoleByIds(String[] lAryId, Long userId)
   {
     ...
     for (String roleId : lAryId)
       if (!StringUtil.isEmpty(roleId))
         this.userRoleDao.delUserRoleByIds(userId, Long.valueOf(Long.parseLong(roleId)));
   }
