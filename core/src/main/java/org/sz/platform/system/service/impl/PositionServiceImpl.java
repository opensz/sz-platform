...
public class PositionServiceImpl extends BaseServiceImpl<Position> implements
	...
	public Position getParentPositionByParentId(long parentId) {
		Position parent = (Position) this.positionDao.getById(Long
				.valueOf(parentId));
	...
	public void updateChildrenNodePath(Position father,
			List<Position> childrenList) {
		...
		while (it.hasNext()) {
			...
			if ((parent != null) && (list != null) && (list.size() > 0)) {
				for (Position gt : list)
					if (gt.getPosId().longValue() != father.getPosId()
							.longValue()) {
						gt.setParentId(parent.getPosId());
						gt.setNodePath(parent.getNodePath() + gt.getPosId()
								+ ".");
						gt.setDepth(Integer.valueOf(parent.getDepth()
								.intValue() + 1));
						update(gt);
					}
			}
	...
	public Map<Long, List<Position>> coverMapData(Long rootId,
			List<Position> instList) {
	...
	private List<Position> getChildListByDicId(Long parentId,
			Map<Long, List<Position>> dataMap) {
		List list = new ArrayList();

		List<Position> dicList = (List) dataMap.get(Long.valueOf(parentId
				.longValue()));
	...
	public void delByIds(Long[] ids) {
		...
		int len$ = arr$.length;
		for (int i$ = 0; i$ < len$; i$++) {
			long posId = arr$[i$].longValue();
			Position gt = (Position) this.positionDao.getById(Long
					.valueOf(posId));
			...
			updateChildrenNodePath(getParentPositionByParentId(0L),
					childrenList);
		}
	}
	...
	public List<Position> getAllChildByParentId(long parentId) {
		...
		for (int i = 0; i < childSize; i++)
			if (((Position) ChildList.get(i)).getIsLeaf().intValue() != 1) {
				List MoreList = getAllChildByParentId(((Position) ChildList
						.get(i)).getPosId().longValue());
	...
	public void delById(long posId) {
		Position position = (Position) this.positionDao.getById(Long
				.valueOf(posId));
