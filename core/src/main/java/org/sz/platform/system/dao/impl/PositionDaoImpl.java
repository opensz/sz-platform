...
import org.sz.platform.system.model.Position;
 ...
 public class PositionDaoImpl extends BaseDaoImpl<Position> implements PositionDao
 {
   ...
   public List<Position> getByNodePath(String nodePath)
   {
     ...
     return getSqlSessionTemplate().selectList(getIbatisMapperNamespace() + ".getAll", p);
   }
 
   public List<Position> getChildByParentId(long parentId)
   {
     return getSqlSessionTemplate().selectList(getIbatisMapperNamespace() + ".getByParentId", Long.valueOf(parentId));
   }
 
   public Integer getChildCountByParentId(long parentId)
   {
     return (Integer)getSqlSessionTemplate().selectOne(getIbatisMapperNamespace() + ".getChildCountByParentId", Long.valueOf(parentId));
   }
