...
public class SysDataSourceServiceImpl extends BaseServiceImpl<SysDataSource>
	...
	public List<Map<String, Object>> testConnectById(Long[] ids) {
		...
		int len$ = arr$.length;
		for (int i$ = 0; i$ < len$; i$++) {
			long id = arr$[i$].longValue();
			SysDataSource sysDataSource = (SysDataSource) this.dao.getById(Long
					.valueOf(id));
	...
	public List<Map<String, Object>> testConnectByForm(
		...
		try {
			...
			DriverManager.getConnection(sysDataSource.getUrl(),
					sysDataSource.getUserName(), sysDataSource.getPassword())
					.close();

			connectResult.put("success", Boolean.valueOf(true));
		} catch (ClassNotFoundException e) {
			connectResult.put("msg",
					"ClassNotFoundException: " + sysDataSource.getDriverName());
