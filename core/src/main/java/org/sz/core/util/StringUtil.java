...
import org.apache.commons.lang.StringUtils;
 ...
 public class StringUtil
 {
   ...
   /** 字符添加的位置 */
   public static final String LOCATIONL = "L";
   public static final String LOCATIONR = "R";
   public static final String LOCATIONLR = "LR";
   ...
   public static void main(String[] args)
   {
	 ...
	 * @return
	 */
	public static String format(String content, String... params) {
		int i = 0;
	...
	public static String join(String[] strs, String separator, String additive,
		...
		if (location.equals(LOCATIONLR)) {
			buf = new StringBuffer(length * strs[0].length()
					+ (additive.length() * 2) * length).append(additive)
					.append(strs[0]).append(additive);
		} else if (location.equals(LOCATIONL)) {
			buf = new StringBuffer(length * strs[0].length()
					+ (additive.length() * 2) * length).append(additive)
					.append(strs[0]);

		} else if (location.equals(LOCATIONR)) {
			buf = new StringBuffer(length * strs[0].length()
					+ (additive.length() * 2) * length).append(strs[0]).append(
					additive);
		...
		for (int i = 1; i < length; i++) {
			if (location.equals(LOCATIONLR)) {
				buf.append(separator).append(additive).append(strs[i]).append(
						additive);
			} else if (location.equals(LOCATIONL)) {
				buf.append(separator).append(additive).append(strs[i]);
			} else if (location.equals(LOCATIONR)) {
				buf.append(separator).append(strs[i]).append(additive);
			}
		}
		return buf.toString();
	}
