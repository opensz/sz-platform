...
public class ProcessDiagramGenerator {
	...
	static {
		...
		activityDrawInstructions.put(StartEvent.class,
				new ActivityDrawInstruction() {
					...
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BpmnModel bpmnModel, FlowNode flowNode) {
						GraphicInfo graphicInfo = bpmnModel
								.getGraphicInfo(flowNode.getId());
						...
						if (intermediateCatchEvent.getEventDefinitions() != null
							...
							if (intermediateCatchEvent.getEventDefinitions()
								...
								processDiagramCanvas.drawCatchingSignalEvent(
										...
										(int) graphicInfo.getY(),
										(int) graphicInfo.getWidth(),
										(int) graphicInfo.getHeight(), false);
				...
				new ActivityDrawInstruction() {
					...
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
						...
						if (throwEvent.getEventDefinitions() != null
								&& throwEvent.getEventDefinitions().size() > 0) {
		...
		activityDrawInstructions.put(Task.class, new ActivityDrawInstruction() {
			...
			public void draw(ProcessDiagramCanvas processDiagramCanvas,
					BpmnModel bpmnModel, FlowNode flowNode) {
				GraphicInfo graphicInfo = bpmnModel.getGraphicInfo(flowNode
						.getId());
				...
				new ActivityDrawInstruction() {
					...
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
						...
						processDiagramCanvas.drawServiceTask(
								flowNode.getName(), (int) graphicInfo.getX(),
								(int) graphicInfo.getY(),
								(int) graphicInfo.getWidth(),
								(int) graphicInfo.getHeight());
					}
				...
				new ActivityDrawInstruction() {
					...
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
						...
						if (boundaryEvent.getEventDefinitions() != null
							...
							} else if (boundaryEvent.getEventDefinitions().get(
									0) instanceof ErrorEventDefinition) {
					...
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
						...
						if (!graphicInfo.isExpanded()) {
							processDiagramCanvas.drawCollapsedSubProcess(
									...
									(int) graphicInfo.getY(),
									(int) graphicInfo.getWidth(),
									(int) graphicInfo.getHeight(), true);
						} else {
							processDiagramCanvas.drawExpandedSubProcess(
									flowNode.getName(),
									(int) graphicInfo.getX(),
	...
	public static InputStream generatePngDiagram(BpmnModel bpmnModel) {
		return generateDiagram(bpmnModel, "png",
	...
	public static InputStream generateJpgDiagram(BpmnModel bpmnModel) {
		return generateDiagram(bpmnModel, "jpg",
				Collections.<String> emptyList());
	}
	...
	public static InputStream generateDiagram(BpmnModel bpmnModel,
		...
		ProcessDiagramCanvas canvas = generateDiagram(bpmnModel,
	...
	public static InputStream generateDiagram(BpmnModel bpmnModel,
		...
		return generateDiagram(bpmnModel, highLightedActivities,
				highLightedFlows).generateImage(imageType);
	}

	protected static void drawActivity(
			ProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
		...
		if (drawInstruction != null) {
			...
			if (flowNode instanceof Activity) {
				...
				if (multiInstanceLoopCharacteristics != null) {
					multiInstanceSequential = multiInstanceLoopCharacteristics
							.isSequential();
		...
		for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
			...
			for (int i = 1; i < graphicInfoList.size(); i++) {
				...
				boolean highLighted = (highLightedFlows.contains(sequenceFlow
						.getId()));
				boolean drawConditionalIndicator = (i == 1)
				...
				if (i < graphicInfoList.size() - 1) {
					...
					processDiagramCanvas.drawSequenceflow(
							...
							drawConditionalIndicator, highLighted);
					if (!drawedLabel) {
						GraphicInfo labelGraphicInfo = bpmnModel
								.getLabelGraphicInfo(sequenceFlow.getId());
						if (labelGraphicInfo != null) {
							int middleX = (int) (((previousGraphicInfo.getX() + labelGraphicInfo
									.getX()) + (graphicInfo.getX() + labelGraphicInfo
									.getX())) / 2);
							int middleY = (int) (((previousGraphicInfo.getY() + labelGraphicInfo
									.getY()) + (graphicInfo.getY() + labelGraphicInfo
									.getY())) / 2);
							middleX += 15;
							processDiagramCanvas.drawLabel(
									sequenceFlow.getName(), middleX, middleY,
		...
		if (flowNode instanceof FlowElementsContainer) {
			for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode)
				...
				if (nestedFlowElement instanceof FlowNode) {
					drawActivity(processDiagramCanvas, bpmnModel,
	...
	private static void drawHighLight(
			ProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
	...
	protected interface ActivityDrawInstruction {
		...
		void draw(ProcessDiagramCanvas processDiagramCanvas,
				BpmnModel bpmnModel, FlowNode flowNode);

	}
	
	public static InputStream generateDiagramHasStatus(BpmnModel bpmnModel,
			String imageType, List<TaskNodeStatus> lStatus) {

		List<String> highLightedActivities = new ArrayList<String>();
		ProcessDiagramCanvas canvas = (ProcessDiagramCanvas)generateDiagram(bpmnModel,
				highLightedActivities, Collections.<String> emptyList());
		GraphicInfo graphicInfo = null;
		Color color = null;
		for(TaskNodeStatus nodeStatus: lStatus){
			if(TaskOpinion.STATUS_INIT == nodeStatus.getLastCheckStatus()) continue;
			graphicInfo = bpmnModel.getGraphicInfo(nodeStatus.getTaskKey());
			color = TaskOpinion.getColorByStatus(nodeStatus.getLastCheckStatus());
			drawHighLight(canvas, graphicInfo, color);
		}	
		return canvas.generateImage(imageType);
	}

	protected static void drawHighLight(
			ProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo,
			Color color) {
		processDiagramCanvas.drawHighLight((int) graphicInfo.getX(),
				(int) graphicInfo.getY(), (int) graphicInfo.getWidth(),
