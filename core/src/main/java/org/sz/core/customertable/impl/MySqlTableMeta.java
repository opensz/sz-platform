...
public class MySqlTableMeta extends BaseTableMeta {
	...
	private List<ColumnModel> getColumnsByTableName(String tableName) {
		...
		Map map = new HashMap();
		List list = this.jdbcHelper
				.queryForList(sql, map, new MySqlColumnMap());
		return list;
	}
	...
	private TableModel getTableModel(final String tableName) {
		...
		TableModel tableModel = (TableModel) this.jdbcHelper.queryForObject(
				sql, null, new RowMapper() {
					public TableModel mapRow(ResultSet rs, int row)
							throws SQLException {
						TableModel tableModel = new TableModel();
						String comments = rs.getString("table_comment");
						comments = MySqlTableMeta.getComments(comments,
								tableName);
	...
	public Map<String, String> getTablesByName(String tableName) {
		if (StringUtil.isNotEmpty(tableName))
			this.sqlAllTable = (this.sqlAllTable + " and table_name like '%"
					+ tableName + "%'");
		this.jdbcHelper.setCurrentDb(this.currentDb);
		Map parameter = new HashMap();
		List list = this.jdbcHelper.queryForList(this.sqlAllTable, parameter,
				new RowMapper() {
	...
	public static String getComments(String comments, String defaultValue) {
		...
		if (idx > -1) {
			comments = StringUtil.trimSufffix(
					comments.substring(0, idx).trim(), ";");
		}
		if (StringUtil.isEmpty(comments)) {
