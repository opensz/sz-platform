...
//		mv.addObject("systemId", sysUser.getSystemId());
	...
	public ModelAndView copy(HttpServletRequest request) throws Exception {
		...
		if (systemId != null) {
			SubSystem subSystem = (SubSystem) this.subSystemService
					.getById(systemId);
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		...
		if (roleId.longValue() != 0L) {
			...
			if (systemIdTemp != null) {
				SubSystem subSystem = (SubSystem) this.subSystemService
						.getById(systemIdTemp);
				String sysAlias = subSystem.getAlias();
				String roleAlias = sysRole.getAlias().replaceFirst(
						sysAlias + "_", "");
				sysRole.setAlias(roleAlias);
			}
		} else {
		...
		return getAutoView().addObject("sysRole", sysRole)
				.addObject("subsystemList", list)
				.addObject("returnUrl", returnUrl)
				.addObject("systemId", systemId);
	}
	...
	public ModelAndView get(HttpServletRequest request,
		...
		if (sysRole.getSystemId() != null) {
			subsystem = (SubSystem) this.subSystemService.getById(sysRole
					.getSystemId());
		}

		return getAutoView().addObject("sysRole", sysRole).addObject(
				"subsystem", subsystem);
	}
...
//		}
	...
	public List<SysRole> getAll(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		List list = this.sysRoleService.getAll(new WebQueryFilter(request,
				false));
		return list;
	}
