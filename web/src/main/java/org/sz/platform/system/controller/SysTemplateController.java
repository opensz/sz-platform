...
public class SysTemplateController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,
		...
		List list = this.sysTemplateService.getAll(new WebQueryFilter(request,
				"sysTemplateItem"));
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		Long templateId = Long.valueOf(RequestUtil.getLong(request,
				"templateId"));
		String returnUrl = RequestUtil.getPrePage(request);
		SysTemplate sysTemplate = null;
		if (templateId.longValue() != 0L)
			sysTemplate = (SysTemplate) this.sysTemplateService
					.getById(templateId);
		else {
			sysTemplate = new SysTemplate();
		}
		return getAutoView().addObject("sysTemplate", sysTemplate).addObject(
				"returnUrl", returnUrl);
	}
	...
	public ModelAndView get(HttpServletRequest request,
		...
		SysTemplate sysTemplate = (SysTemplate) this.sysTemplateService
