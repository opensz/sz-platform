...
public class SysUserParamController extends BaseController {
	...
	public ModelAndView editByUserId(HttpServletRequest request,
		...
		List list = this.sysUserParamService.getAll(new WebQueryFilter(request,
				"sysUserParamItem"));

		ModelAndView mv = getAutoView().addObject("sysUserParamList", list)
				.addObject("sysParamList", sysParamList)
				.addObject("userId", Long.valueOf(userId))
				.addObject("user", user).addObject("returnUrl", returnUrl);
	...
	public void saveByUserId(HttpServletRequest request,
		...
		try {
			...
			e.printStackTrace();
			ResultMessage message = new ResultMessage(0, "编辑人员参数属性失败"
					+ e.getMessage());
