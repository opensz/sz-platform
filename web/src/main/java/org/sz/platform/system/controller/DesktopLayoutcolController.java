...
import org.sz.platform.system.service.DesktopColumnService;
import org.sz.platform.system.service.DesktopLayoutService;
import org.sz.platform.system.service.DesktopLayoutcolService;
import org.sz.platform.system.service.DesktopMycolumnService;
 ...
 public class DesktopLayoutcolController extends BaseController
 {
   ...
   public ModelAndView list(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     List list = this.desktopLayoutcolService.getAll(new WebQueryFilter(request, "desktopLayoutcolItem"));
     ...
     throws Exception
   {
     ...
     desktopLayoutcol = this.desktopLayoutcolService.editData(id, desktopColumnmap, desktopLayoutmap);
     return getAutoView().addObject("desktopLayoutcol", desktopLayoutcol).addObject("returnUrl", returnUrl).addObject("desktopColumnmap", desktopColumnmap).addObject("desktopLayoutmap", desktopLayoutmap);
   }
     ...
     throws Exception
   {
     ...
     DesktopLayoutcol desktopLayoutcol = (DesktopLayoutcol)this.desktopLayoutcolService.getById(Long.valueOf(id));
     ...
     throws Exception
   {
     ...
     List layoutcolList = this.desktopLayoutcolService.layoutcolData(layoutId);
     DesktopLayout desktopLayout = (DesktopLayout)this.desktopLayoutService.getById(layoutId);
     Map mapData = this.desktopMycolumnService.getDefaultDeskDataById(layoutId, ctxPath);
     String html = this.freemarkEngine.mergeTemplateIntoString("desktop/getDeskTop.ftl", mapData);
 
     return getAutoView().addObject("desktopColumnList", list).addObject("html", html).addObject("desktopLayout", desktopLayout);
   }
 
   @RequestMapping({"getLayoutcolData"})
   @ResponseBody
   public List<DesktopLayoutcol> getLayoutcolData(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     Long layoutId = Long.valueOf(RequestUtil.getLong(request, "id"));
     ...
     throws Exception
   {
     ...
     for (int i = 0; i < jsoA.size(); i++) {
       ...
       desktopLayoutcol.setColumnId(Long.valueOf(jsoO.getLong("columnId")));
