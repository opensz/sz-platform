...
import org.sz.platform.system.service.SysUserService;
 ...
 public class MessageReplyController extends BaseController
 {
     ...
     throws Exception
   {
     ...
     MessageSend messageSend = (MessageSend)this.sendService.getById(messageId);
     ...
     ModelAndView mv = getAutoView().addObject("replyList", list).addObject("userId", userId).addObject("messageSend", messageSend).addObject("returnUrl", returnUrl);
   ...
   public ModelAndView edit(HttpServletRequest request) throws Exception {
     ...
     return getAutoView().addObject("messageReply", messageReply).addObject("subject", messageSend.getSubject()).addObject("content", messageSend.getContent()).addObject("returnUrl", returnUrl);
   }
     ...
     throws Exception
   {
     ...
     MessageReply messageReply = (MessageReply)this.replyService.getById(Long.valueOf(id));
   ...
   public void reply(HttpServletRequest request, HttpServletResponse response, MessageReply messageReply) throws Exception {
     try { SysUser sysUser = ContextUtil.getCurrentUser();
