...
import org.sz.platform.service.system.SysOfficeTemplateService;
 ...
 public class SysOfficeTemplateController extends BaseController
 {
   ...
   public ModelAndView selector(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     List list = this.sysOfficeTemplateService.getAll(new WebQueryFilter(request, "sysOfficeTemplateItem"));
     ModelAndView mv = getAutoView().addObject("sysOfficeTemplateList", list);
   ...
   public ModelAndView edit(HttpServletRequest request) throws Exception {
     ...
     if (id.longValue() != 0L)
       sysOfficeTemplate = (SysOfficeTemplate)this.sysOfficeTemplateService.getById(id);
     else {
     ...
     throws Exception
   {
     ...
     SysOfficeTemplate sysOfficeTemplate = (SysOfficeTemplate)this.sysOfficeTemplateService.getById(Long.valueOf(id));
     return getAutoView().addObject("sysOfficeTemplate", sysOfficeTemplate);
   }
   @RequestMapping({"saveTemplate"})
   @Action(description="保存更新系统模板")
   public void saveTemplate(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception { String resultMsg = "";
     try {
       ...
       if (it.hasNext()) {
         ...
         if (!isOfficeFile(extName)) {
           writeResultMessage(response.getWriter(), new ResultMessage(0, "上传的模板文件格式必须为：" + doc));
           return;
         }
         ...
         sysOfficeTemplate.setId(Long.valueOf(id));
         sysOfficeTemplate.setPath(((SysOfficeTemplate)this.sysOfficeTemplateService.getById(Long.valueOf(id))).getPath());
         this.sysOfficeTemplateService.update(sysOfficeTemplate);
         resultMsg = "更新系统模板成功";
       }
       writeResultMessage(response.getWriter(), new ResultMessage(1, resultMsg));
     } catch (Exception e) {
       writeResultMessage(response.getWriter(), new ResultMessage(0, "操作系统模板失败"));
     }
   }
