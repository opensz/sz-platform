...
import org.sz.platform.service.system.GlobalTypeService;
 ...
 public class DictionaryController extends BaseController
 {
     ...
     throws Exception
   {
     ...
     if (isAdd == 1) {
       ...
       if (isRoot == 1) {
         ...
         Dictionary parentDic = (Dictionary)this.dictionaryService.getById(dicId);
         dictionary.setParentId(dicId);
     ...
     mv.addObject("dictionary", dictionary).addObject("isAdd", Integer.valueOf(isAdd));
   ...
   public void sort(HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     try { this.dictionaryService.updSn(lAryId);
   ...
   public List<Dictionary> getByTypeId(HttpServletRequest request)
   {
     ...
     List list = this.dictionaryService.getByTypeId(typeId.longValue(), true);
     return list;
   }
   ...
   public Map<String, Object> getMapByNodeKey(HttpServletRequest request)
   {
     String nodeKey = RequestUtil.getString(request, "nodeKey");
     Map map = new HashMap();
     GlobalType globalType = this.globalTypeService.getByDictNodeKey(nodeKey);
     List list = this.dictionaryService.getByTypeId(globalType.getTypeId().longValue(), false);
     ...
     throws Exception
   {
     ...
     try {
       this.dictionaryService.move(Long.valueOf(targetId), Long.valueOf(dragId), moveType);
