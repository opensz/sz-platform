...
import org.sz.platform.bpm.util.BpmUtil;
 ...
 public class BpmNodeRuleController extends BaseController
 {
   ...
   public ModelAndView list(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     List list = this.bpmNodeRuleService.getAll(new WebQueryFilter(request, "bpmNodeRuleItem"));
   ...
   public ModelAndView edit(HttpServletRequest request) throws Exception {
     ...
     BpmDefinition bpmDefinition = this.bpmDefinitionService.getByActDefId(actDefId);
 
     List nodeList = new ArrayList();
     nodeList.add(nodeId);
     Map activityList = BpmUtil.getTranstoActivitys(defXml, nodeList);
 
     BpmNodeSet bpmNodeSet = this.bpmNodeSetService.getByActDefIdNodeId(actDefId, nodeId);
 
     bpmNodeRule.setActDefId(actDefId);
     bpmNodeRule.setNodeId(nodeId);
 
     ModelAndView mv = getAutoView().addObject("activityList", activityList).addObject("nodeName", nodeName).addObject("bpmNodeRule", bpmNodeRule).addObject("deployId", deployId).addObject("actDefId", actDefId).addObject("nodeId", nodeId).addObject("bpmNodeSet", bpmNodeSet).addObject("defId", bpmDefinition.getDefId());
 
     return mv;
   }
 
   @RequestMapping({"getByDefIdNodeId"})
   public void getByDefIdNodeId(HttpServletRequest request, HttpServletResponse response)
     throws IOException
   {
     String actDefId = RequestUtil.getString(request, "actDefId");
     String nodeId = RequestUtil.getString(request, "nodeId");
     PrintWriter out = response.getWriter();
     List ruleList = this.bpmNodeRuleService.getByDefIdNodeId(actDefId, nodeId);
     String str = JSONArray.fromObject(ruleList).toString();
     out.print(str);
   }
 
   @RequestMapping({"sortRule"})
   public void sortRule(HttpServletRequest request, HttpServletResponse response)
     throws IOException
   {
     try
     {
     ...
     throws Exception
   {
     ...
     BpmNodeRule bpmNodeRule = (BpmNodeRule)this.bpmNodeRuleService.getById(Long.valueOf(id));
   ...
   @ResponseBody
   public String updateIsJumpForDef(HttpServletRequest request, HttpServletResponse response) throws IOException { String nodeId = request.getParameter("nodeId");
     String actDefId = request.getParameter("actDefId");
     String isJumpForDef = request.getParameter("isJumpForDef");
     this.logger.debug("nodeId:" + nodeId + " actDefId:" + actDefId + " isJumpForDef:" + isJumpForDef);
     this.bpmNodeSetService.updateIsJumpForDef(nodeId, actDefId, new Short(isJumpForDef));
     return "{success:true}";
   }
 }
