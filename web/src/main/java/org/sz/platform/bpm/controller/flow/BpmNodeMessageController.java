...
import org.sz.platform.system.service.SysTemplateService;
 ...
 public class BpmNodeMessageController extends BaseController
 {
   ...
   public ModelAndView list(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     List list = this.bpmNodeMessageService.getAll(new WebQueryFilter(request, "bpmNodeMessageItem"));
   ...
   public ModelAndView edit(HttpServletRequest request) throws Exception {
     List tempList = this.sysTempplateService.getAll(new WebQueryFilter(request, "sysTemplateItem"));
     ...
     Map dataMap = this.messageService.getMapByActDefIdNodeId(actDefId, nodeId);
 
     Message mailModel = (Message)dataMap.get(org.sz.platform.system.model.Message.MAIL_TYPE);
     if (mailModel != null)
     {
     ...
     Message mobileModel = (Message)dataMap.get(org.sz.platform.system.model.Message.MOBILE_TYPE);
     if (mobileModel != null)
     {
       receiver_mobile = mobileModel.getReceiver();
       templateId_mobile = mobileModel.getTemplateId();
     }
     Message innerModel = (Message)dataMap.get(org.sz.platform.system.model.Message.INNER_TYPE);
     if (innerModel != null)
     {
     ...
     return getAutoView().addObject("actDefId", actDefId).addObject("nodeId", nodeId).addObject("subject_mail", subject_mail).addObject("receiver_mail", receiver_mail).addObject("copyTo_mail", copyTo_mail).addObject("bcc_mail", bcc_mail).addObject("templateId_mail", templateId_mail).addObject("receiver_mobile", receiver_mobile).addObject("templateId_mobile", templateId_mobile).addObject("subject_inner", subject_inner).addObject("receiver_inner", receiver_inner).addObject("templateId_inner", templateId_inner).addObject("tempList", tempList);
   }
     ...
     throws Exception
   {
     ...
     BpmNodeMessage bpmNodeMessage = (BpmNodeMessage)this.bpmNodeMessageService.getById(Long.valueOf(id));
