...
import org.sz.platform.bpm.service.form.BpmFormTableService;
 ...
 public class BpmFormTableFormController extends BaseFormController
 {
     ...
     throws Exception
   {
     ...
     try
     {
       if (table.getTableId() == null) {
         if (this.bpmFormTableService.isTableNameExisted(table.getTableName())) {
           msg = "表名已存在";
         ...
         boolean isExist = this.bpmFormTableService.isTableNameExistedForUpd(table.getTableId(), table.getTableName());
         if (isExist) {
           msg = "输入的表名在系统中已经存在!";
           writeResultMessage(response.getWriter(), msg, 0);
           return;
   ...
   public void saveExtTable(HttpServletRequest request, HttpServletResponse response)
     throws Exception
   {
     String tableJson = request.getParameter("table");
     String fieldsJson = request.getParameter("fields");
     ...
     try
     {
       if (table.getTableId() == null) {
         ...
         if (this.bpmFormTableService.isTableNameExternalExisted(tableName, dsAlias)) {
           msg = "表名已存在";
           writeResultMessage(response.getWriter(), msg, 0);
           return;
         }
