...
import org.sz.platform.bpm.util.FormDataUtil;
 ...
 public class BpmFormHandlerController extends BaseController
 {
   ...
   public ModelAndView edit(HttpServletRequest request)
     throws Exception
   {
     Long formDefId = Long.valueOf(RequestUtil.getLong(request, "formDefId"));
     ...
     if (formDefId.longValue() != 0L) {
       ...
       String html = this.service.obtainHtml(bpmFormDef, ContextUtil.getCurrentUserId(), pkValue, "", "", "");
   ...
   public ModelAndView bizForm(HttpServletRequest request)
     throws Exception
   {
     ...
     if(runId > 0L){
    	 ...
    	 BpmNodeSet bpmNodeSet = bpmNodeSetService.getBySetType(processRun.getDefId(), BpmNodeSet.SetType_GloabalForm);
     ...
     if (formKey.longValue() != 0L) {
       bpmFormDef = this.bpmFormDefService.getDefaultVersionByFormKey(formKey);
       String html = this.service.obtainHtml(bpmFormDef, ContextUtil.getCurrentUserId(), pkValue, "", "", "");
       bpmFormDef.setHtml(html);
     }
     return getAutoView().addObject("bpmFormDef", bpmFormDef).addObject("returnUrl", returnUrl);
