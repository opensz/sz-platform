...
public class BpmNodeUserController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,
		...
		List list = this.bpmNodeUserService.getAll(new WebQueryFilter(request,
				"bpmNodeUserItem"));
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		Long userNodeId = Long.valueOf(RequestUtil.getLong(request,
				"userNodeId"));
		String returnUrl = RequestUtil.getPrePage(request);
		BpmNodeUser bpmNodeUser = null;
		if (userNodeId != null)
			bpmNodeUser = (BpmNodeUser) this.bpmNodeUserService
					.getById(userNodeId);
		else {
			bpmNodeUser = new BpmNodeUser();
		}
		return getAutoView().addObject("bpmNodeUser", bpmNodeUser).addObject(
				"returnUrl", returnUrl);
	}
