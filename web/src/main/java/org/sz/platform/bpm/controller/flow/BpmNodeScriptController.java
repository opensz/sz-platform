...
import org.sz.platform.bpm.service.flow.BpmNodeScriptService;
 ...
 public class BpmNodeScriptController extends BaseController
 {
     ...
     throws Exception
   {
     ...
     for (int i = 0; i < aryScriptType.length; i++) {
       ...
       if ((StringUtil.isEmpty(script)) || 
         (!StringUtil.isNotEmpty(script))) continue;
     ...
     try
     {
       ...
       writeResultMessage(response.getWriter(), "保存节点脚本失败:" + e.getMessage(), 0);
     }
   }
   ...
   public ModelAndView edit(HttpServletRequest request) throws Exception {
     ...
     Map map = this.bpmNodeScriptService.getMapByNodeScriptId(nodeId, actDefId);
     return getAutoView().addObject("map", map).addObject("type", type).addObject("nodeId", nodeId).addObject("actDefId", actDefId).addObject("defId", defId);
   }
 }
