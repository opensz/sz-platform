...
import org.sz.core.query.PageUtils;
...
public class PageNav {
	...
	private String getPara(HttpServletRequest request) {
		...
		while (e.hasMoreElements()) {
			...
			if ((value == null) || (value.equals("")))
				continue;
			params = params + "&" + key + "=" + getValueByParameter(key);
		}
	...
	public String getNumber() {
		...
		List<Integer> list = PageUtils.getPageNumbers(
				this._pv.getCurrentPage(), this._pv.getTotalPage(), this.count);
		StringBuffer sb = new StringBuffer();
		sb.append("<div class='pageNav'>");
		String url = this.strUrl + "?page=%1$s" + this._strPara;
		String curTmplate = "<span class='spanCurPage'><a href='" + url
				+ "'>%1$s</a></span>";
		String tmplate = "<span class='spanPage'><a href='" + url
				+ "'>%1$s</a></span>";
		for (Integer it : list) {
			if (currentPage == it.intValue())
				sb.append(String.format(curTmplate,
						new Object[] { Integer.valueOf(it.intValue()) }));
			else
				sb.append(String.format(tmplate,
						new Object[] { Integer.valueOf(it.intValue()) }));
		}
		sb.append("</div>");
		return sb.toString();
	...
	public String getPage(boolean allRecord, boolean pagesInfo,
			boolean pageNav, boolean jumpPage) throws Exception {
		StringBuffer sb = new StringBuffer();
		sb.append("<div class='pageNav'>");
		if (allRecord)
	...
	private String showJumpPage() throws Exception {
		...
		sb.append(" 转到第<input type=\"text\" name=\"page"
				+ i
		...
		sb.append("location.assign('" + this.strUrl + "?page='" + "+value+'"
				+ this._strPara + "')");
		sb.append("}");
		sb.append("}");
		sb.append("</script>");
	...
	private String showPageInfo() throws Exception {
		return "<span class='pageInfo'> 第" + this._pv.getCurrentPage() + "/"
				+ this._pv.getTotalPage() + "页 </span>";
	}
	...
	private String showPageNav() throws Exception {
		...
		if (this._pv.getTotalPage() > 1)
			sb.append("<a href=\"" + this.strUrl + "?page=1" + this._strPara
					+ "\">第一页</a>");
		else
			sb.append("<a disabled='true' >第一页</a>");
		if (this._pv.isHasPreviousPage())
			sb.append("<a href=\"" + this.strUrl + "?page="
					+ this._pv.getPreviousPage() + this._strPara + "\">上一页</a>");
		else
			sb.append("<a disabled='true' >上一页</a>");
		if (this._pv.isHasNextPage()) {
			sb.append("<a href=\"" + this.strUrl + "?page="
					+ this._pv.getNextPage() + this._strPara + "\">下一页</a>");
		} else {
			sb.append("<a disabled='true' >下一页</a>");
		}
		if (this._pv.getTotalPage() > 1)
			sb.append("<a href=\"" + this.strUrl + "?page="
					+ this._pv.getTotalPage() + this._strPara + "\">最末页</a>");
		else
	...
	private String showTotalRecord() throws Exception {
		return "<span class='totalRecord'>共" + this._pv.getTotalCount()
				+ "条记录 </span>";
	}
}
