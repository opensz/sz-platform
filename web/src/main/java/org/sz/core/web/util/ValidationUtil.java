...
public class ValidationUtil {
	...
	private static ValidForm getForm(String formName, Locale local) {
		...
		for (Field fld : list) {
			...
			String displayName = ResourceUtil
					.getText(arg.getKey(), null, local);
	...
	private static void getRuleByField(Field field, ValidField vFld,
			Locale local) {
	...
	private static void handRule(Field field, Rule rule, Locale local) {
		...
		case required: // ValidEnum.required:
			tipInfo = ResourceUtil.getText("errors.required", displayName,
					local);
		...
		case maxlength:
			...
			tipInfo = ResourceUtil.getText("errors.maxlength", aryMaxlength,
					local);
		...
		case minlength:
			...
			tipInfo = ResourceUtil.getText("errors.minlength", aryMinlength,
					local);
		...
		case rangelength:
			...
			tipInfo = ResourceUtil.getText("errors.rangelength",
					aryRangeLength, local);
		...
		case qq:
			...
			break;
		case chinese:
			tipInfo = ResourceUtil
					.getText("errors.chinese", displayName, local);
			rule.setRuleInfo("true");
			break;
		case chrnum:
	...
	public static String getJs(String roleForm, Locale local)
		...
		FreemarkEngine freemaker = (FreemarkEngine) SpringContextHolder
				.getBean(FreemarkEngine.class);
