...
<script type="text/javascript" src="${ctx}/jslib/jquery/jquery.form.js"></script>
	<script type="text/javascript">
			...
			if(top!=this){//当这个窗口出现在iframe里，表示其目前已经timeout，需要把外面的框架窗口也重定向登录页面
				  top.location='<%=request.getContextPath()%>/login/login_itsmS3.xht';
			}
...
<script type="text/javascript">
	...
	function showCenter() {
		replaceCss(oldNavId);
		document.getElementById('orgId').value=20027;
		var objSelect=document.getElementById('j_orgname_suborg2');
		document.getElementById('org_select').style.display = '';
		 for (var i = 0; i < objSelect.options.length; i++) {        
        if (objSelect.options[i].value == document.getElementById('orgId').value) {        
	...
	function hiddenCenter() {
		replaceCss(oldNavId);
		document.getElementById('orgId').value=20000;
	...
	function setOrgId(value){
		document.getElementById('orgId').value=value;
        			...
        			<table width="300" border="0">
        				  <tr id="org_select">
        				  	...
        				  	<td ><select style="width:150px;"
										id="j_orgname_suborg2" onchange="setOrgId(this.value);">
									<c:forEach items="${sysOrgList}" var="model">
										<option value="${model.orgId}">${model.orgName}</option>
									</c:forEach>
								 </select>
							</td>
        				  </tr>
                          ...
                          <tr align="left">
							...
							<tr>
								<td align="left" colspan="2">
										<font color="red"><div id="msg"><c:out value="${sessionScope.SPRING_SECURITY_LAST_EXCEPTION}"/></div></font>
								</td>
							</tr>
