...
function preview(){
	...
	var url=__ctx + '/platform/bpm/bpmNodeUserUplow/getByUserId.xht?json='+json;
	url=url.getNewUrl();
...
<div position="center">
			...
			<div class="panel">
				...
				<div class="panel-body">
						...
						<form id="sysUserParamForm" method="post" action="${ctx }/platform/system/sysUserParam/saveByUserId.xht">
							...
							<table id="demensionItem" cellpadding="1" cellspacing="1"  class="table-grid">
								...
								<tbody>
										...
										<tr>
										<td>
											<input  type="checkbox" class="pk" name="valueId">
										</td>
										
										<td>
											<select name="demensionId" style="width: 70%;">
										...
										<td>
											<select name="upLowType" style="width: 70%;" onchange="validUpLowType(this);">
												<c:forEach items="${uplowtypeList}" var="t" >
													<option value="${t.value }"<c:if test="${t.value==0 }">selected="selected"</c:if> >${t.key }</option>
												</c:forEach>
											</select>
										</td>
										<td>
											
											<select  name="upLowLevel" style="width: 70%;display: none;" onchange="validUpLowLevel(this);" >
