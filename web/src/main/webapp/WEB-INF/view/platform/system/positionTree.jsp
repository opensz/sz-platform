<%@ page language="java" contentType="text/html; charset=UTF-8"	pageEncoding="UTF-8" import="org.sz.platform.system.model.Position"%>
...
<head>
	...
	<script type="text/javascript">
		...
		//当前访问
		var returnUrl="${ctx}/platform/system/position/list.xht";
		$(function()
		{
			loadTree();
			layout();
		...
		function loadTree(){
			...
			var setting = {
				...
				async: {
					enable: true,
					url:"${ctx}/platform/system/position/getChildTreeData.xht",
		...
		function onDrop(event, treeId, treeNodes, targetNode, moveType) {
			if(targetNode){
				...
				if(originalIds.length>1){
					...
					$.ajax({
						type: 'POST',
						url:"${ctx}/platform/system/position/move.xht?targetId="+targetId+"&originalIds="+originalIds,
		...
		function sortPos(targetNode,posIds){
			$.ajax({
				type: 'POST',
				url:"${ctx}/platform/system/position/sort.xht?posIds="+posIds,
				success: function(result){
					reAsyncChild(targetNode);
				}
			});
		...
		*/
		...
		function zTreeOnLeftClick(event, treeId, treeNode){
			returnUrl="${ctx}/platform/system/position/list.xht?Q_nodePath_S="+treeNode.nodePath+"&parentId="+treeNode.posId;
		...
		function addNode(){
			...
			$("#listFrame").attr("src","${ctx}/platform/system/position/edit.xht?parentId="+getSelectNode().posId+"&returnUrl="+returnUrl);
		};
		//编辑岗位
		function editNode(){
			var selectNode=getSelectNode();
			...
			$("#listFrame").attr("src","${ctx}/platform/system/position/edit.xht?posId="+posId+"&returnUrl="+returnUrl);
		};
		//删除岗位
		function delNode(){
			var selectNode=getSelectNode();
			...
			if(posId==<%=Position.ROOT_ID%>){$.ligerMsg.info('该节点为系统节点 ,不充许该操作');return;}
		 	var callback = function(rtn) {
				if(rtn){
					$.ajax({
						type: 'POST',
						url:"${ctx}/platform/system/position//del.xht?posId="+posId+"&returnUrl="+returnUrl,
						success: function(result){
		...
		function users(){
			...
			var posId=selectNode.posId;
			if(posId==<%=Position.ROOT_ID%>){$.ligerMsg.info('该节点为系统节点 ,不充许该操作');return;}
			$("#listFrame").attr("src","${ctx}/platform/system/userPosition/edit.xht?posId="+posId+"&returnUrl="+returnUrl);
		};
		//选择岗位
...
<div id="layout">
	...
	<div position="center">
		<iframe id="listFrame" src="${ctx}/platform/system/position/list.xht" frameborder="no" width="100%" height="100%"></iframe>
	</div>
