...
<head>
	...
	<script type="text/javascript">
		...
		function getInsertUser(userId,fullname){
			var tr='<tr>';
			tr+='<td>';
			tr+='	<input type="checkbox" class="pk"	name="userPosId" value="">';
			tr+='</td>';
			
			tr+='<td>';
			tr+='   <input type="hidden" name="userId" id="'+ userId +'"	 value="'+userId+'" readonly="readonly">';
			tr+=fullname;
			tr+='</td>';
			tr+='<td>';
			tr+='	<input type="checkbox" name="isPrimary"	 checked="checked" value="'+userId+'" >';
			tr+='</td>';
			tr+='<td >';
...
<div class="panel">
		...
		<div class="panel-body" title="岗位">
		
			<form id="positionForm" method="post" action="save.xht">
			   	...
			   	<table id="userPositionItem" class="table-grid table-list" id="0" cellpadding="1" cellspacing="1">
			    	...
			    	<c:forEach items="${userPositionList}" var="userPositionItem">
			    		<tr>
			    			...
			    			<td >
			    				<input type="checkbox" name="isPrimary"	 value="${userPositionItem.userId}"  <c:if test="${userPositionItem.isPrimary==0}">checked="checked"</c:if> >
			    			</td>
			    			<td >
