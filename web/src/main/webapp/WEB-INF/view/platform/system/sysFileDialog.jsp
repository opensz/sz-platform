...
<script type="text/javascript" src="${ctx}/js/sz/platform/system/FileMenu.js"></script>
...
<script type="text/javascript">
	...
	var url="${ctx}/platform/system/globalType/getPersonType.xht";
	...
	function treeClick(treeNode) {
		...
		var url = "${ctx}/platform/system/sysFile/selector.xht?typeId=" + typeId +"&isSingle=${isSingle}";
	 ...
	 */
	function fileUpload(){
		var url="${ctx}/platform/system/sysFile/getFileUpload.xht";
...
<body>
	<div class="panel-top">
		<div class="tbar-title">
			<span class="tbar-label">上传图片</span>
		</div>
	</div>
	<div id="defLayout">
		...
		<div position="center"><div class="l-layout-header">附件分类列表<div style="float:right">
			<input type="button" value="上  传" onclick="fileUpload();" /></div></div>			
			<iframe id="fileFrame" name="fileFrame" height="90%" width="100%" frameborder="0" src="${ctx}/platform/system/sysFile/selector.xht?isSingle=${isSingle}"></iframe>
		</div>
		...
		<div position="bottom"  class="bottom" >
				<a href='#' class='button'  onclick="selectFile()" ><span class="icon ok"></span><span >选择</span></a>
