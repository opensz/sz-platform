...
<head>
	...
	<script type="text/javascript">
		...
		function searchObjectList(){
			...
			var url=__ctx +"/platform/form/bpmFormDialog/getByDsObjectName.xht";
			...
			$.post(url, { dsName:dsName, objectName: objectname,istable:istable },function(data) {
				...
				if(istable=="1"){
					...
					for(key in tables ){
						selList.append("<option value='"+ key+"'>"+ key +"("+tables[key] +")" +"</option>" );
					}
				}
				//视图的处理
				else{
		...
		function showSettingDialog(dsName,objectname,istable,style,id){
			var url="setting.xht?dsName=" +dsName +"&objectName=" + objectname +"&istable=" + istable +"&style=" +style +"&id=" + id;
...
<div class="panel">
		...
		<div class="panel-body">
				<form id="bpmFormDialogForm" method="post" action="save.xht">
					<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
						...
						<tr>
							...
							<td>
								<input type="radio" name="issingle" value="1" <c:if test="${bpmFormDialog.issingle==1}">checked="checked"</c:if>>单选
								<input type="radio" name="issingle" value="0" <c:if test="${bpmFormDialog.issingle==0}">checked="checked"</c:if>>多选
						...
						<tr>
							...
							<td colspan="3" valign="top">
								...
								<c:choose>
									<c:when test="${bpmFormDialog.id==0}">
										<br>
										<select id="objname" name="objname" size="10" style="width:150px;">
										</select>
