...
<script type="text/javascript">
	...
	function getPermission() {
		$.post('getPermissionByFormDefId.xht?formDefId=${bpmFormDef.formDefId}', function(data) {
			permission = {};
	...
	function insertHtml(editor, node) {
		...
		if (node.fieldType) {
			...
			if (!controls[node.fieldName]) {
				showSelectTemplate('selectTemplate.xht?tableId=' + node.tableId + '&single=1', function(win) {
					var form = $(document.getElementById('select').contentDocument);
			...
			if (!html) {
				showSelectTemplate('selectTemplate.xht?tableId=' + node.tableId + '&single=1',
					function(win) {
						var form = $(document.getElementById('select').contentDocument);
						var templateId = form.find('select[name=tableTemplateId]').val();
						win.close();
	 ...
	 *   table数据
	 */
	var iconFolder = '${ctx }/themes/img/tree/';
	var editor;
	var tableId = '${bpmFormDef.tableId}';

	function getAllFields() {
		$.post('getAllFieldsByTableId.xht?tableId=${bpmFormDef.tableId}', function(data) {
