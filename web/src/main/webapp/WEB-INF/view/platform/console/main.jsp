<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="org.sz.platform.system.model.Resources"%>
<!DOCTYPE>
<html>
<head>
...
<link href="${ctx}/styles/${styleName}/css/Aqua/css/ligerui-all.css"
	type="text/css" rel="stylesheet" name="styleTag" title="index">
<link href="${ctx}/styles/${styleName}/css/web.css" type="text/css"
	rel="stylesheet" name="styleTag" title="index">
<link href="${ctx}/styles/${styleName}/css/index.css" type="text/css"
	rel="stylesheet" name="styleTag" title="index">
<link href="${ctx}/styles/${styleName}/css/select.css" type="text/css"
	rel="stylesheet" name="styleTag" title="index">
...
<script type="text/javascript">
				...
				.mouseup(
						function(login) {
							if ($(login.target).hasClass("more")) {
								...
								if (prehref)
									addToTab(prehref, $(login.target).text(),
											$(login.target).attr('resid'));
								else
									location.href = $(login.target)
											.attr('href');
	...
	function loadMenu() {
		$("#leftTree").empty();
		//一次性加载
		$.post("${ctx}/platform/console/getSysRolResTreeData.xht", function(
				result) {
			aryTreeData = result;
	...
	function changeSkin(styleName) {
		...
		$.post(__ctx + "/login/changeStyle.xht", {
			'styleName' : styleName
		}, function(response) {
			var json = response;
	...
	function setLink(linktitle, styleName, obj) {
		...
		$('link[rel=stylesheet]:[title=' + linktitle + ']', obj).each(
				function() {
					...
					if (updatelogo) {
						var newlogo = $('#logoImg').attr("src").replace(substr,
								styleName);
...
#pageloading {
	...
	left: 0px;
	top: 0px;
	background: white url('${ctx}/themes/img/ligericons/loading.gif')
		no-repeat center;
	width: 100%;
	height: 100%;
	height: 700px;
...
<body style="padding:0px;">
	...
	<div id="layoutMain" style="width:100%">
		<div position="left"
			title="<img src='${ctx}/themes/img/icon/home.png' >  ${currentSystem.sysName }">
					...
					<c:choose>
						<c:when test="${currentSystem.isLocal==1}">
							<iframe frameborder="0" name="home"
								src="${ctx}${currentSystem.homePage}"></iframe>
						</c:when>
						<c:otherwise>
							<iframe frameborder="0" name="home"
								src="${currentSystem.defaultUrl}${currentSystem.homePage}"></iframe>
						</c:otherwise>
					</c:choose>
				</c:if>
			</div>
