 ...
 * 功能颜色选择器。
 * 使用方法:
 * $.showcolor("按钮ID","接收颜色的文本框ID");
 */
jQuery.extend({
    showcolor:function(txtid,btnid){showColorPanel(btnid,txtid);  }
});
...
function initColor(){
    $("body").append('<div id="colorpanel" style="position: absolute; display: none;z-index:100;"></div>');
    var colorTable='';
    for(i=0;i<2;i++){
        for(j=0;j<6;j++){
