Namespace.register("org.sz.platform.desktop");
/**
 * 获取栏目模板
 */
org.sz.platform.desktop.getColumnTemp= function(cols,width,showTable){
...
org.sz.platform.desktop.homeTemp= function(cols,width){	
  $.post(
	__ctx+"/platform/system/desktopMycolumn/getMycolumnData.xht",
	function(jsonData){
		var colshtml=org.sz.platform.desktop.getColumnTemp(cols,width,0);		
		$("#colshtml").append(colshtml);
		$(jsonData).each(function (i) {  
            //同步处理布局中的数据链接  
            columnUrl=jsonData[i].columnUrl;
            $.post( 
                __ctx + "/platform/system/desktopColumn/getTemp.xht?id="+jsonData[i].columnId,
                function (jsonStr) { 
                    //创建布局窗格
                	var htt,htd;                	
 ...
 * 获取栏目数据的模板
 */
org.sz.platform.desktop.myTemp= function(cols,columnId,width,showTable,desktop){
		    var columnUrl;
		    $('#news').attr('href',"news.xht?id="+columnId);
		    $('#change').attr('href',"change.xht?id="+columnId);
		    var colshtml;
	        $.ajax({  
	            type: "POST",  
	            url:__ctx + "/platform/system/desktopMycolumn/getMycolumnData.xht",
	            beforeSend: function (XMLHttpRequest) { $("#londing").show(); },  
	            success: function (jsonData) {  
	            	//加载布局模板
	                colshtml=org.sz.platform.desktop.getColumnTemp(cols,width,showTable,desktop);
	                $("#colshtml").append(colshtml);
	                ...
	                $(jsonData).each(function (i) {  
	                    ...
	                    columnUrl=jsonData[i].columnUrl;
	                    $.ajax({  
	                        url: __ctx + "/platform/system/desktopColumn/getTemp.xht?id="+jsonData[i].columnId,  
	                        async: false,  
 ...
 * 保存布局
 */
org.sz.platform.desktop.saveCol= function(layoutID,filename,layoutCols){
		var jsonSaveStr = "[";
		// 获取布局数据
		...
		$.ajax({
			type : "POST",
			url : __ctx + "/platform/system/"+filename+"/saveCol.xht",
			data : {
			...
			success : function(result) {
				var msg = new org.sz.form.ResultMessage(result);
				if (msg.isSuccess()) {
					ref = true;
