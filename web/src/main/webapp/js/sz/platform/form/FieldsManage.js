...
FieldsManage=function(){
	...
	this.isFieldExist=function(name){
		for(var i=0;i<this.Fields.length;i++){
			...
			//判断选择控件。（用户，组织，岗位，角色)
			if(field.controlType==4 || field.controlType==5 || field.controlType==6 ||  field.controlType==7 ||  field.controlType==8 ||  field.controlType==16){
	...
	this.getHtml=function(){
		...
		for(var i=0;i<this.Fields.length;i++){
			...
			sb.append("<td style='text-align:center;'>"+this.getCheckBox(field,"isQuery")+"</td>");
 ...
 * 添加列。
 */
TableRow.addColumn=function(){
	
	ColumnDialog({isAdd:true,fieldManage:TableRow.fieldManage, callBack:function(field){
 ...
 * 编辑字段
 */
TableRow.editField=function(fieldName){
	var tmpField=TableRow.fieldManage.getFieldByName(fieldName);
	if(tmpField==null) return;
	
	ColumnDialog({isAdd:false,allowEditColName:TableRow.allowEditColName, fieldManage:TableRow.fieldManage,field:tmpField,callBack:function(field){
