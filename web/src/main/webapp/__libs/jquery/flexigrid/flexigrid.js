...
(function($) {
    ...
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            height: 200, //default height 高度
            width: 'auto', //auto width  宽度
            striped: true, //apply odd even stripes  隔行变色
            novstripe: false,  //显示条纹
            minwidth: 30, //min width of columns
            minheight: 80, //min height of columns
            resizable: true, //resizable table  //可调整table大小
            url: false, //ajax url
            method: 'POST', // data sending method
            dataType: 'json', // type of data loaded
            errormsg: '连接错误，请重试！',  //错误提升信息
            usepager: false, //使用分页
            nowrap: true, //
            page: 1, //current page  当前页码
            total: 1, //total pages  当前行数
            useRp: true, //use the results per page select box  分页下列框
            rp: 25, // results per page  //每页显示行数
            rpOptions: [20, 25,30,35, 40],  //分页设置
            title: false,  //显示标题
            pagestat: '显示 {from} 到 {to}， 共 {total} 条' ,//'Displaying {from} to {to} of {total} items',  //显示当前页和总页面的样式
            procmsg: '数据处理中 ...',  //正在处理的提示信息
            query: '',  //搜索查询的条件
            qtype: '',  //搜索查询的类别
            nomsg: '无数据',  //无结果的提示信息
                ...
                if (p.dataType == 'json') {
					...
					(
					 ...
					 function(i, row) {
					     ...
					     var checked=false;			//回填标志
					     if (i % 2 && p.striped) tr.className = 'erow';

					     if (row.id) tr.id = 'row' + row.id;
					     
					     //如果有回填
					     if(row.checked)checked=true;
					     //////checkbox
				         if (p.checkbox) {
								var cth = $('<th/>');
								var str='<input type="checkbox" value="' + $(
										tr).attr('id') + '" dataId='+row.id;
					         ...
					         $('thead tr:first th', g.hDiv).each
							    (
							 	    function() {
							 	        ...
							 	        //设置  修改之前版本的bug 因为row为对象，不存在cell属性。
										$.each(row,function(name,value){
											if(n.name == name){
								 	            td.align = this.align;
	
								 	            //render(cell,row,data,index) 刘冬--最后修改，sm.zhang
								 	            //cell:当前单元格的数据
								 	            //row:当前行数据
								 	            //data:所有数据
								 	            //index:当前行号
...
//							 	        }
            ...
            populate: function(callback) { //get latest data
                ...
                var param = [
					...
					, { name: 'rp', value: p.rp }
					, { name: 'limit', value: p.rp }				//根据DCOM 进行修改
						...
						function() {
							...
							.click(
								function(e) {
								    ...
								    if (obj.href || obj.type) return true;			/////checkbox
						            ...
						            if (p.onClick) p.onClick($(rowData)); 
								}
							)
							.mousedown(
								function(e) {
							...
							.dblclick(
								function(e){
						            ...
						            if (p.onDblclick) p.onDblclick($(rowData)); 
							 })
							;
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'>条件过滤 <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt + "</select> <input type='button' id='search' value='查询' /> <input type='button' id='Clear' value='清除' /></div>");

                $('input[id=search]', g.sDiv).click(function() { g.doSearch(); });
        ...
        if ($('th', g.hDiv).length) {
            ...
            if (p.checkbox) {
         		$('tr', g.hDiv).each(
                 	function () {
                     var cth = $('<td/>');
                     var cthch = $('<input type="checkbox" tag="parentBox"/>');
                     ...
                     cth.addClass("cth").attr({ width: "10",align:"center" }).append(cthch);
                     $(this).prepend(cth);
                 })
     		};
            /////checkbox
	...
	//sm.zhang 增加回调函数
    $.fn.flexReload = function(p,callback) { // function to reload grid

        return this.each(function() {
            if (this.grid && this.p.url) this.grid.populate(callback);
        });
	            	...
	            	if($(this).attr("checked") == "true" || $(this).attr("checked")){
	            		...
	            		if(value !=undefined){
	            			if(value.indexOf("/")>0){
			            		...
			            		select.push(value.substring(0,value.indexOf("/")));
